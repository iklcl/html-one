...
def get_data(params):
	...
	for page in xrange(10):
		...
		html=requests.get('https://www.baidu.com/s',headers=headers,params=data).content
		xiaye=re.findall(r'(下一页)',html)
		...
		for i in href:
			...
			if params[1] in title:
				data=[params[2],params[0],title,i.xpath('a/@href')[0]]
				with codecs.open(u'path2.csv','ab') as f:
					w = csv.writer(f)
					w.writerow(data)
		if len(xiaye)==0:
			break
