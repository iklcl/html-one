...
def get_link(nd,keyword):
	...
	for t in xrange(1,5):
		...
		req=requests.get('http://www.ccgp-neimenggu.gov.cn/zfcgwslave/web/index.php?',headers=headers,params=params).content
		
		lis = re.findall('(\[.*\])',req)

		jd = json.loads(lis[0])[0]
		for i in jd:	
			ti=re.findall('(\d+-\d+-\d+)',i['SUBDATE'])[0]
			...
			if abs((ntime-datime).days)<=nd:
				title=i['TITLE_ALL']
				link='http://www.ccgp-neimenggu.gov.cn/ay_post/post.php?tb_id=%s&p_id=%s'%(i['ay_table_tag'],i['wp_mark_id'])
