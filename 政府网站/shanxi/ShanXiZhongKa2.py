...
#文件名称：ShanXiZhongKa2.py
#作者：huanghong
#创建日期：2017-10-12
#功能描述：收集陕西重卡服务站信息
#网页地址：http://www.sxqc.com/fwzwdRX/index_330.aspx
#处理进度：已完成（待优化）
import time
import requests
...
def get_city():
	data=[[u'北京市',u'北京市'],[u'天津市',u'天津市'],['u上海市',u'上海市'],[u'重庆市',u'重庆市']]
...
def get_data(city):
	...
	if len(jd)!=0:
		for t in jd:
			i = eval(t)
			data=[city[0],city[1],i['title'],i['allDayPhone'],i['address'],i['point']]
			# print data
			savesqlcar_4s(data,u'服务站.db')
def main():
	datas=get_city()
	for i in datas:
		print i[0]+'============='+i[1]
...
def savesqlcar_4s(data,savepoint_name):
    ...
    values("%s","%s","%s","%s","%s","%s","%s")'%(data[0],data[1],data[2],data[3],data[4],data[5],'陕西重型汽车有限公司服务站')
    con.execute(sql)
    con.commit()
    con.close()
main()	    