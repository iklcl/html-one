#-*- coding:utf-8 -*-
#网址：http://www.buick.com.cn/dealer.html
#文件名：bieke.py
#作者: huanghong
#创建日期: 2017-010-25
#功能描述: 上汽通用别克经销商
...
def get_data():
	req=requests.get('http://www.buick.com.cn/api/dealer.aspx',headers=headers).content
	lis = re.findall('(\[.*\])',req)[0]
	js=json.loads(lis)
	for i in js:
		data = [i['provinceName'],i['cityName'],i['dealerName'],i['tel'],i['address'],i['lat'],i['lng']]
		savesql(data,'上汽通用别克经销商.db')
...
def savesql(data,savepoint_name):
    ...
    values("%s","%s","%s","%s","%s","%s","%s","%s")'%(data[0],data[1],data[2],data[3],data[4],data[5],data[6],u'上汽通用别克经销商')
    con.execute(sql)
    con.commit()
    con.close()
