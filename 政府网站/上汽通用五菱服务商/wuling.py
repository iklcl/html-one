...
def savesqlcar_4s(data,savepoint_name):
    ...
    values("%s","%s","%s","%s","%s","%s","%s")'%(data[0],data[1],data[2],data[3],data[4],data[5],'上汽通用五菱服务商')
    con.execute(sql)
    con.commit()
    con.close()		
def get_city():
...
def getP(city):
	...
	print len(jd)
	for cit in jd:
		date=[city[0],city[1],cit['company'],cit['hot_tel'],cit['address'],cit['point']]
		savesqlcar_4s(date,'上汽通用五菱服务商.db')
def main():
	city=get_city()
	for i in city:
		getP(i)	
